...
static void ATL_dJIK0x0x24TT1x1x24_aX_bX
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_dJIK0x0x24TT0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=T, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   #define incAk 24
   const int incAm = ((((lda) << 2)) - 24), incAn = -(Mb*lda);
   const int incBk = (ldb), incBm = -(((ldb) << 4)+((ldb) << 3));
   #define incBn 1
   #define incCm 4
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x24TT1x1x24_aX_bX(k, N, 24, alpha, ca + (Mb*lda), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
