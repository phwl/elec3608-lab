...
static void ATL_dJIK0x0x24NT1x1x4_aX_bX
...
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_dJIK0x0x24NT0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=T, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x24NT1x1x4_aX_bX(k, N, 24, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
