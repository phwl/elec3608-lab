...
static void ATL_dJIK0x0x24NN1x1x24_aX_bX
...
/*
 * matmul with TA=N, TB=N, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   const int incAk = (lda);
   const int incAm = 1 - (((lda) << 4)+((lda) << 3)), incAn = -(Mb);
   #define incBk 24
   ...
   #undef Kb
#endif
void ATL_dJIK0x0x24NN0x0x0_aX_bX
   (const int M, const int N, const int K, const double alpha, const double * ATL_RESTRICT A, const int lda, const double * ATL_RESTRICT B, const int ldb, const double beta, double * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=N, TB=N, MB=0, NB=0, KB=24, 
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 4 - (((lda) << 4)+((lda) << 3)), incAn = -(Mb);
   #define incBk 24
   const int incBm = -24, incBn = (ldb);
   #define incCm 4
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x24NN1x1x24_aX_bX(k, N, 24, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
