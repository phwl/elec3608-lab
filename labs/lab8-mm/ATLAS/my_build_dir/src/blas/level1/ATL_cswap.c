...
void ATL_cswap_xp0yp0aXbX
   (const int, const TYPE*, const int, TYPE*, const int);
void Mjoin(PATLU,swap)
   (const int, const TYPE*, const int, TYPE*, const int);

void ATL_cswap(const int N, const TYPE *X, const int incX,
...
L1:
      if (incx==1 && incy==1)
      {
         Mjoin(PATLU,swap)(N+N, X, 1, Y, 1);
         return;
      }

      ATL_cswap_xp0yp0aXbX(N, X, incx, Y, incy);
   }
}
