...
void ATL_dcpsc(const int, const SCALAR, const TYPE*, const int, TYPE*, const int);
void ATL_dscal(const int, const SCALAR, TYPE*, const int);

void ATL_daxpby(const int N, const SCALAR alpha, const TYPE *X, const int incX,
...
L1:
      if (beta == ATL_rone)
      {
         ATL_daxpy(N, alpha, X, incx, Y, incy);
         return;
      }
      else if (beta == ATL_rzero)
      {
         ATL_dcpsc(N, alpha, X, incx, Y, incy);
         return;
      }
