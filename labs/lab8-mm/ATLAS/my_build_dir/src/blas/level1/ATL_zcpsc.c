...
void ATL_zcpsc_xp0yp0aXbX
   (const int, const SCALAR, const TYPE*, const int, TYPE*, const int);
void ATL_zzero(const int, TYPE*, const int);
void ATL_zcopy(const int, const TYPE*, const int, TYPE*, const int);
void Mjoin(PATLU,cpsc)
   (const int, const TYPE, const TYPE*, const int, TYPE*, const int);

void ATL_zcpsc(const int N, const SCALAR alpha, const TYPE *X, const int incX,
...
L1:
      ...
      ATL_zcpsc_xp0yp0aXbX(N, alpha, X, incx, Y, incy);
   }
   else if ( SCALAR_IS_ONE(alpha) ) ATL_zcopy(N, X, incX, Y, incY);
   else if ( SCALAR_IS_ZERO(alpha) ) ATL_zzero(N, Y, incY);
}
