...
void ATL_caxpby_xp0yp0aXbX
   (const int, const SCALAR, const TYPE*, const int, const SCALAR, TYPE*, const int);
void ATL_caxpy(const int, const SCALAR, const TYPE*, const int, TYPE*, const int);
void ATL_ccpsc(const int, const SCALAR, const TYPE*, const int, TYPE*, const int);
void ATL_cscal(const int, const SCALAR, TYPE*, const int);
void Mjoin(PATLU,axpby)
   (const int, const TYPE, const TYPE*, const int, const TYPE, TYPE*, const int);

void ATL_caxpby(const int N, const SCALAR alpha, const TYPE *X, const int incX,
...
L1:
      if (ibeta == ATL_rzero) /* real beta */
      {
         ...
         {
            ATL_ccpsc(N, alpha, X, incx, Y, incy);
            return;
         }
      ...
      else if (ialpha == ATL_rzero) /* real alpha, cplx beta */
      {
         if (ralpha == ATL_rzero)
         {
            ATL_cscal(N, beta, Y, incy);
            return;
         }
      }

      ATL_caxpby_xp0yp0aXbX(N, alpha, X, incx, beta, Y, incy);
   }
}
